...
- (NSString*)testLinearEquation {
	...
	for (int i = 0; i < 5; i++) {
		int vectorSize = (int)powl(2, i+4);
		
		double t_cpu = test_linearEquation_on_cpu(vectorSize, testCount);
		double t_accelerate = test_linearEquation_on_acclerate(vectorSize, testCount);
